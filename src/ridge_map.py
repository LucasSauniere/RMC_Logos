...
class RidgeMap:
    ...
    def save_svg(
        ...
        line_color="black",
        linewidth=2,
        background_color="#ebeaeb",
        width=1200,
        height=1600,
    ):
        """Save the map as SVG (vector format).

        Perfect for logos, prints, and scalable graphics!
        ...
        label_size : int
            fontsize of the label
        line_color : string
            Color for the lines (must be a string, not callable for SVG)
        linewidth : float
            Width of each line in the map
        background_color : color
            For the background (hex string or color name)
        width : int
            SVG width in pixels
        height : int
        ...
        if callable(line_color):
            raise ValueError("SVG export does not support callable line_color. Please use a string color.")

        # Convert background_color to hex if it's a tuple
        if isinstance(background_color, tuple):
            bg_hex = self._rgb_to_hex(background_color)
        else:
        ...
        if label_color is None:
            label_color = line_color
        if isinstance(label_color, tuple):
            label_color = self._rgb_to_hex(label_color)
        ...
        # Calculate scaling
        num_lines, num_pts = values.shape
        scale_x = width / num_pts
        scale_y = height / (num_lines * 6 + 12)  # Match the matplotlib spacing
        ...
        for idx, row in enumerate(values):
            ...
            for i in range(len(x)):
                if not np.isnan(y[i]):
                    svg_x = x[i] * scale_x
                    svg_y = height - ((y[i] - y_base.min() + 6) * scale_y)
                    svg_points.append((svg_x, svg_y))

            # Draw line
            if len(svg_points) >= 2:
                # Create filled polygon (line + fill)
                fill_points = svg_points.copy()
                # Add baseline points to close the polygon
                fill_points.append((svg_points[-1][0], height - ((y_base - y_base.min() + 6) * scale_y)))
                fill_points.append((svg_points[0][0], height - ((y_base - y_base.min() + 6) * scale_y)))
        ...
        if label:
            ...
            label_svg_x = label_x * width
            label_svg_y = label_y * height
            
            # SVG text (note: multi-line support is basic)
            lines = label.split('\n')
            for i, line in enumerate(lines):
                text = dwg.text(
                    line,
                    insert=(label_svg_x, label_svg_y + i * label_size * 1.2),
                    fill=label_color,
